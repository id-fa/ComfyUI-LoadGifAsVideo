...
- **`SaveAsGif` letterboxes rather than stretches, and writes the bars as GIF transparency.** `width`/`height` are a canvas, not a target size: the frame is scaled to fit inside it keeping its aspect ratio and centred, and the leftover is transparent. Either at 0 derives from the other, both at 0 leaves the source alone, so the widgets default to a no-op. Do not add a "stretch to fill" mode without being asked — silently changing an animation's aspect ratio is the thing this design is avoiding.
...
- **`optimize=False`**, because Pillow's optimize rebuilds the palette from the colors a frame actually uses, undoing a deliberately shared global one.
...
- **`palette_size` drops to 255 when letterboxed**, because the transparent entry has to fit in the same 256-entry table.
