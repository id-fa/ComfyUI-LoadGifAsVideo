...
| `halftone-mask-inverted`, `halftone-square-mask-inverted`, `halftone-diamond-mask-inverted`, `halftone-brick-mask-inverted` | Ordered | The same screens with the two regions swapped: the picture shows **through** the dots and the `halftone_ink` colour fills the ground between them. Pixel for pixel the complement of the matching `-mask` method. See [Mask screens](#mask-screens). |
...
| `halftone-mask-inverted`, `halftone-square-mask-inverted`, `halftone-diamond-mask-inverted`, `halftone-brick-mask-inverted` | 順序ディザ | 上の 4 つの絵が見える部分とマスクされる部分を入れ替えたもの。ドットの**中**から元絵が見え、ドットの外側が `halftone_ink` の色で塗り潰されます。対応する `-mask` とピクセル単位で完全に補集合です。[マスクスクリーン](#マスクスクリーン)を参照。 |
